package info.sleeplessacorn.nomagi.common.item;

import info.sleeplessacorn.nomagi.ModObjects;
import info.sleeplessacorn.nomagi.common.block.BlockChair;
import info.sleeplessacorn.nomagi.common.block.BlockChair.Half;
...
public class ItemBlockChair extends ItemBlockBase {
    ...
    public boolean placeBlockAt(
        ...
        if (world.getBlockState(pos.up()).getBlock().isReplaceable(world, pos.up())) {
            ...
            if (world.getBlockState(pos.down()).getBlock().isReplaceable(world, pos.down())) {
    ...
    private boolean placeChairAt(World world, EntityPlayer player, BlockPos posUpper, BlockPos posLower) {
        if (!world.isRemote) {
            ...
            world.playSound(null, posLower, sound.getPlaceSound(), SoundCategory.BLOCKS,
                    (sound.getVolume() + 1.0F) / 2.0F, sound.getPitch() * 0.8F);
            return world.setBlockState(posUpper, state.withProperty(block.getPropertyEnum(), Half.UPPER)) &&
                    world.setBlockState(posLower, state.withProperty(block.getPropertyEnum(), Half.LOWER));
        }
        return false;
    }
