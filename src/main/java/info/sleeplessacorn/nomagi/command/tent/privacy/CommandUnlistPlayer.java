...
public class CommandUnlistPlayer extends CommandBase {
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        EntityPlayer toUnlist = getPlayer(server, sender, args[1]);

        TentWorldSavedData tentData = TentWorldSavedData.getData(toUnlist.getEntityWorld());
        Tent tent = tentData.getTent(getCommandSenderAsPlayer(sender).getGameProfile().getId());
        if (tent == null)
            return;

        tent.getPrivacy().listPlayer(listType, toUnlist.getGameProfile().getId());
        tent.markDirty();
    }
