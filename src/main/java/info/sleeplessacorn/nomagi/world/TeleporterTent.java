...
public class TeleporterTent extends Teleporter {
    ...
    public static boolean teleportToDimension(EntityPlayer player, Pair<Integer, BlockPos> dimPos) {
        return dimPos != null &&
                teleportToDimension(player, dimPos.getKey(), dimPos.getValue().getX() + 0.5, dimPos.getValue().getY(),
    ...
    public static boolean teleportToDimension(EntityPlayer player, int dimension, double x, double y, double z) {
        ...
        worldServer.getMinecraftServer().getPlayerList().transferPlayerToDimension((EntityPlayerMP) player, dimension,
                new TeleporterTent(worldServer, x, y, z));
